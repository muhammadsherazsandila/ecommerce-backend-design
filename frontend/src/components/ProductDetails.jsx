...
import { Star, Heart, MessageSquare, ShoppingBag, ShieldCheck, Globe, ChevronRight, Check, Loader2, ArrowLeft } from 'lucide-react';
  ...
  if (!product) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center">
        <h3 className="text-lg font-bold text-[#1C1C1C] mb-2">Product not found</h3>
        <button onClick={() => setPage('listing')} className="text-primary font-medium hover:underline">
          Back to products
        </button>
      </div>
    );
  ...
  return (
    <div className="container py-4">
      ...
      <div className="flex items-center gap-2 text-[#8B96A5] text-sm mb-6">
        ...
        <ChevronRight className="w-4 h-4" />
        <span className="text-[#1C1C1C] font-normal truncate max-w-[200px]">{product.name}</span>
      </div>
      ...
      <div className="bg-white border border-[#DEE2E7] rounded-lg p-5 lg:p-8 flex flex-col lg:flex-row gap-8 mb-8 shadow-sm">
        ...
        <div className="lg:w-[450px] flex-shrink-0">
          <div className="border border-[#DEE2E7] rounded-lg p-8 mb-4 flex items-center justify-center bg-[#F7F7F7] aspect-square overflow-hidden group relative">
            <img
              src={productImage}
              alt={product.name}
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-4 mb-4 flex-wrap">
            <div className="flex items-center gap-1">
              {Array(5).fill(0).map((_, i) => (
                <Star key={i} size={16} className={i < Math.round(product.rating) ? "fill-[#FF9017] text-[#FF9017]" : "text-[#D1D3D3]"} />
              ))}
          ...
          <div className="bg-[#FFF0DF] p-4 rounded-lg flex flex-wrap gap-8 items-center mb-6">
            ...
            {discount > 0 && (
              <>
                ...
                <div className="flex flex-col">
                  <span className="text-lg font-bold text-[#00B517]">Save {discount}%</span>
                  <span className="text-xs text-[#505050]">${(product.oldPrice - product.price).toFixed(2)} off</span>
                </div>
              </>
            )}
          </div>
          ...
          <div className="space-y-4 mb-8">
            ...
            <div className="grid grid-cols-3 lg:grid-cols-4 gap-4 text-sm border-t border-[#DEE2E7] pt-4">
              ...
              <span className="col-span-2 lg:col-span-3 text-[#00B517] font-medium">{product.shipping}</span>
            </div>
          </div>
          ...
          <div className="bg-white border border-[#DEE2E7] rounded-lg p-5">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[#505050] text-sm">{product.brand || 'Official Store'}</span>
              </div>
            </div>
            ...
            <div className="p-6 lg:p-8">
              ...
              <table className="w-full text-sm lg:text-base mb-8 rounded overflow-hidden border-collapse">
                <tbody>
                  ...
                  {product.brand && (
                    <tr className="border border-[#DEE2E7]">
                      <td className="w-1/3 bg-[#F7FAFC] p-3 text-[#505050] font-medium border-r border-[#DEE2E7]">Brand</td>
                      <td className="p-3 text-[#505050]">{product.brand}</td>
                    </tr>
                  )}
                  <tr className="border border-[#DEE2E7]">
                    <td className="w-1/3 bg-[#F7FAFC] p-3 text-[#505050] font-medium border-r border-[#DEE2E7]">Rating</td>
                    <td className="p-3 text-[#505050]">{product.rating.toFixed(1)} / 5.0</td>
                  </tr>
                  <tr className="border border-[#DEE2E7]">
                  ...
                  <tr className="border border-[#DEE2E7]">
                    <td className="w-1/3 bg-[#F7FAFC] p-3 text-[#505050] font-medium border-r border-[#DEE2E7]">Shipping</td>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
            {relatedProducts.slice(0, 5).map((item) => (
              ...
              >
                ...
                <div className="flex flex-col">
                  <span className="text-[#505050] text-sm line-clamp-2 group-hover:text-primary transition-colors">{item.name}</span>
                  <span className="text-[#1C1C1C] text-sm font-bold mt-1">${item.price.toFixed(2)}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-gradient-to-r from-primary to-[#005ADE] rounded-lg p-8 flex flex-col md:flex-row items-center justify-between gap-6 mb-8 text-white relative overflow-hidden">
        <div className="relative z-10 text-center md:text-left">
          ...
          <p className="opacity-80 text-sm">Exclusive deals available for all registered members.</p>
        </div>
        <button
